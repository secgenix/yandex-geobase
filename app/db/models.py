...
object_labels = Table(
    'object_labels',
    Base.metadata,
    Column('object_id', Integer, ForeignKey('geo_objects.id', ondelete='CASCADE'), primary_key=True),
...
class User(Base):
    ...
    geo_objects_created = relationship('GeoObject', foreign_keys='GeoObject.created_by', back_populates='creator')
    geo_objects_updated = relationship('GeoObject', foreign_keys='GeoObject.updated_by', back_populates='updater')
...
class AuditLog(Base):
    ...
    @staticmethod
    def log(db, user_id: int, action: str, resource_type: str = None,
            resource_id: int = None, changes: dict = None, status: str = 'success',
...
class ObjectComment(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    object_id = Column(Integer, ForeignKey('geo_objects.id', ondelete='CASCADE'), nullable=False)
